  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ invoice }) => {
      ...
      this.productOrderService
        ...
        .subscribe((resBody: IProductOrder[]) => {
          if (!invoice.invoiceId || !invoice.invoiceId.id) {
            this.invoiceids = resBody;
          } else {
            this.productOrderService
              .find(invoice.invoiceId.id)
              .pipe(
                map((subRes: HttpResponse<IProductOrder>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: IProductOrder[]) => (this.invoiceids = concatRes));
          }
        });
    });
  }
